# 현재 API 호출 횟수를 아끼고 속도를 높이기 위해, 각 검색어당 sort="sim"(유사도순)으로 상위 100개(1페이지)만 가져와서 채점하고 있습니다.
# 마이너 뉴스까지 모두 가져오고 싶다면 sort="date"(최신순)로 설정하고, 기사 작성일(pubDate)이 '어제'로 바뀔 때까지 페이지(start)를 계속 넘기며 오늘 발행된 모든 기사를 가져오도록 변경 필요
